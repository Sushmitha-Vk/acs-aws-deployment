...
locals {
  ...
  application_name = "${var.resource-prefix}-alfresco-repo-${var.autoscaling-group-image-id}"
}
...
### set alfresco-globlal.properties ###
...
sudo sed -i 's/SOLR-SECURE-COMMS/${var.solr-secureComms}/g' /opt/alfresco-content-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/ENABLE-TIMESTAMP-PROPAGATION/${var.enable-timestamp-propagation}/g' /opt/alfresco-content-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/THUMBNAIL-GENERATE/${var.thumbnail-generate}/g' /opt/alfresco-content-services/tomcat/shared/classes/alfresco-global.properties
...
sudo sed -i 's/@@instance_name@@/${local.instance_name}/g' /opt/alfresco-content-services/tomcat/shared/classes/stagemonitor.properties
sudo sed -i 's/@@application_name@@/${local.application_name}/g' /opt/alfresco-content-services/tomcat/shared/classes/stagemonitor.properties
sudo sed -i 's/@@elasticsearch_url@@/${local.elasticsearch_url}/g' /opt/alfresco-content-services/tomcat/shared/classes/stagemonitor.properties
...
sudo systemctl set-environment 'CATALINA_OPTS="${var.catalina-opts}"'
sudo systemctl start tomcat 

### Share instance ###
sudo sed -i 's/MQ-USER/${local.mq_user}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/MQ-PASSWORD/${local.mq_password}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's|MQ-FAIL-OVER|${local.mq_failover}|g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/INTERNAL-LB-URL/${local.internal_lb_url}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/LB-URL/${local.lb_url}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/DB-DRIVER/${var.rds-driver}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/DB-USERNAME/${var.rds-username}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/DB-PASSWORD/${var.rds-password}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/DB-NAME/${var.rds-name}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/DB-URL/${local.db_url}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/S3-BUCKET-LOCATION/${var.s3-bucket-location}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/S3-BUCKET-NAME/${var.resource-prefix}-repo-bucket/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/SOLR-SECURE-COMMS/${var.solr-secureComms}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/ENABLE-TIMESTAMP-PROPAGATION/${var.enable-timestamp-propagation}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/THUMBNAIL-GENERATE/${var.thumbnail-generate}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/JOD-CONVERTER-PORT/${var.jod-converter-port}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/PDF-RENDERER-PORT/${var.pdf-renderer-port}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/IMAGE-MAGICK-PORT/${var.image-magick-port}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/TIKA-PORT/${var.tika-port}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo sed -i 's/SHARED-FILE-STORE-PORT/${var.shared-file-store-port}/g' /opt/alfresco-share-services/tomcat/shared/classes/alfresco-global.properties
sudo systemctl enable share_tomcat
sudo systemctl start share_tomcat    
...
resource "aws_autoscaling_group" "asg" {
  name = "${aws_launch_configuration.repo-lcfg.id}-repo-asg"
  ...
  target_group_arns = ["${aws_alb_target_group.repo-target-group.arn}","${aws_alb_target_group.share-target-group.arn}"]
...
resource "aws_alb_target_group" "repo-target-group" {
  ...
  tags = {
    Name = "${var.resource-prefix}-repo-tg"
    Environment = "${var.resource-prefix}"
    Owner = "${var.resource-owner}"
    Customer = "${var.resource-customer}"
  }
...
resource "aws_alb_target_group" "share-target-group" {
  name     = "${var.resource-prefix}-share-target-group"
  port     = 8081
  protocol = "HTTP"
  vpc_id   = "${var.vpc-id}"
  ...
  tags = {
    Name = "${var.resource-prefix}-share-tg"
    Environment = "${var.resource-prefix}"
    Owner = "${var.resource-owner}"
    Customer = "${var.resource-customer}"
  }
...
resource "aws_alb_listener_rule" "acs-share-rule" {
  ...
  action {
    type = "forward"
    target_group_arn = "${aws_alb_target_group.share-target-group.arn}"
  }
  condition {
    field  = "path-pattern"
    values = ["/share*"]
